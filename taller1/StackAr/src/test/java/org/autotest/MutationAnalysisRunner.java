...
public abstract class MutationAnalysisRunner {
    ...
    private final Map<String, Integer> mutantsCreated = new HashMap<>();
    ...
    public boolean runTestsForCurrentClassBeingTested() {
        String description = getMutantDescription(classBeingTested.getSimpleName());
        System.out.println("-> Corriendo tests de " + this.getClass().getSimpleName() +
                " para la clase: " + classBeingTested.getSimpleName() +
                " (" + description + ")");

        String mutationClass = description.split(" ", 2)[0];
        if (mutantsCreated.containsKey(mutationClass)) {
            mutantsCreated.put(mutationClass, mutantsCreated.get(mutationClass) + 1);
        } else {
            mutantsCreated.put(mutationClass, 1);
        }
    ...
    private void printMutantStatistics() {
        ...
        System.out.println("-> Mutantes generados por clase:");
        for(String description: mutantsCreated.keySet()) {
            System.out.println(" | " + description + ": " + mutantsCreated.get(description).toString());
        }
